...
namespace MacintoshBot.Models.Role
{
    public class LevelRoleRepository : ILevelRoleRepository
    {
        ...
        public async Task<(Status status, RoleDTO role)> Create(RoleDTO role)
        {
            ...
            if (existingRole.status == Status.Found) return (Status.Conflict, existingRole.role);
            ...
            if (createdRole.Entity == null) return (Status.Error, role);
        ...
        {
            var role = await _context.LevelRoles.FirstOrDefaultAsync(r =>
                r.RefName.ToLower().Equals(refName.ToLower()) && r.GuildId == guildId);
            if (role == null) return (Status.BadRequest, null);
        ...
        public async Task<(Status status, RoleDTO role)> GetHighestRank(ulong guildId)
        {
            ...
            if (!roles.Any()) return (Status.BadRequest, null);
        ...
        public async Task<(Status status, RoleDTO role)> GetLevelRoleFromTime(DateTimeOffset joinedSince, ulong guildId)
        {
            ...
            if (result.status != Status.Found) return (Status.BadRequest, null);
            var levelRoles = await GetAllLevel(guildId);
            if (!levelRoles.Any()) return (Status.BadRequest, null);
            if (result.days >= 365)
            {
                var pro = levelRoles.FirstOrDefault(l => l.Rank == 2);
                if (pro != null) return (Status.Found, pro);
            }

            if (result.days >= 100)
            {
                var intermediate = levelRoles.FirstOrDefault(l => l.Rank == 1);
                if (intermediate != null) return (Status.Found, intermediate);
            }

            var scrub = levelRoles.FirstOrDefault(l => l.Rank == 0);
            if (scrub != null) return (Status.Found, scrub);
        ...
        {
            var role = (await GetAllLevel(guildId)).FirstOrDefault(r =>
                member.Roles.Select(mr => mr.Id).Contains(r.RoleId));
            if (role == null) return (Status.BadRequest, null);
        ...
        public async Task<(Status status, RoleDTO role)> GetLevelNext(int rank, ulong guildId)
        {
            ...
            if (nextLevel == null) return (Status.BadRequest, null);
        ...
        public (Status status, int days) GetDays(DateTimeOffset joinedSince)
        {
            ...
            if (joinedSince > now) return (Status.BadRequest, 0);
